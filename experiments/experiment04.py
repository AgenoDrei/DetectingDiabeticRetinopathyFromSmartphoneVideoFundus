...
def run():
    images = load_images(path='/home/simon/Videos/Anomaly Dataset/raw_images', img_type='png')
...
def detect_circles(img:np.array, radius_reduction=20) -> np.array:
    ...
    #detect small lens
    small_circles = cv2.HoughCircles(img_blur, cv2.HOUGH_GRADIENT, 1, img.shape[0] / 8, minRadius=400, maxRadius=500, param1=HOUGH_PARAM, param2=HOUGH_PARAM)
    ...
    if circle is None:                   #detect large lens
        large_circles = cv2.HoughCircles(img_blur, cv2.HOUGH_GRADIENT, 1, img.shape[0] / 8, minRadius=500, maxRadius=600, param1=HOUGH_PARAM, param2=HOUGH_PARAM)
    ...
    if circle is not None:
        ...
        show_image_row([output, cv2.cvtColor(mask, cv2.COLOR_GRAY2BGR)], name='circles', time=1000)
        return mask
